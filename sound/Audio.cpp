...
int InitDirectSound( HWND hwnd)
{
   ...
   FillMemory(sdd, sizeof(SOUNDDEVICEDESC) * MAX_SOUND_DEVICE, 0); // alacn
   ...
   for( int i = 0; i < iTotalSoundDevices; i++ ) {
	  ...
	  if( lpds->GetCaps( &sdd[i].DSC ) != DS_OK )
	  {
		  lpds->Release();
		  continue;
	  }
...
SKIPSECONDARY:
   ...
   if( hres != DS_OK ) {
      // alacn ->
      if(lpdsSecondary) lpdsSecondary->Release();
	  lpdsPrimary->Release();
	  lpDS->Release();
	  // <- alacn
...
void InitAudioSystem(HWND hw, HANDLE log) // alacn
{
	// alacn ->
	hlog = log;
	xCamera = 1024;
...
BOOL CALLBACK EnumerateSoundDevice( GUID* lpGuid, LPSTR lpstrDescription, LPSTR lpstrModule, LPVOID lpContext)
{
   ...
   if(iTotalSoundDevices >= MAX_SOUND_DEVICE) return FALSE; // alacn
...
void Audio_Shutdown()
{
   ...
   // alacn ->
   if(lpdsSecondary) lpdsSecondary->Release();
   if(lpdsPrimary) lpdsPrimary->Release();
   lpDS->Release();
   iSoundActive = 0;
   // <- alacn
}
