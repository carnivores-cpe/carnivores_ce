...
void SetupRes()
{
	...
	if (OptRes==7) { WinW =1600; WinH=1050; }	
}
...
void ProcessCommandLine()
{
 ...
 ScentMode = FALSE;
  for (int a=0; a<__argc; a++) {
	 ...
	 if (strstr(s,"-cover")) ScentMode = TRUE;
...
void AddShipTask(int cindex)
{
  ...
  float score = (float)DinoInfo[Characters[cindex].CType][Characters[cindex].SUBAI].BaseScore;
  ...
  if (!Tranq) {
   ...
   PrintLog(DinoInfo[Characters[cindex].CType][Characters[cindex].SUBAI].Name);
   PrintLog("\n");
  }  
}
...
void InitShip(int cindex)
{
	...
	Ship.DeltaY = 2048.f + DinoInfo[cptr->CType][cptr->SUBAI].ShDelta * cptr->scale;
...
void HideWeapon()
{
  ...
  if (wptr->state == 0) {   
	  ...
	  wptr->yplace = 0;
	  wptr->xplace = 0;
	  wptr->ironsights = 1; //Default to 'going rest' animation
      return;
  }
...
{
	for (int c=0; c<32; c++) {
		DinoInfo[c][0].Scale0 = 800;
		DinoInfo[c][0].ScaleA = 600;
		DinoInfo[c][0].ShDelta = 0;
	}
	// ======== Player Model info ========= //
	DinoInfo[AI_to_CIndex[AI_PLAYERS]][0].AI = AI_to_CIndex[AI_PLAYERS];
	wsprintf(DinoInfo[AI_to_CIndex[AI_PLAYERS]][0].FName, "huntdat\\bigfoot.car");
	DinoInfo[AI_to_CIndex[AI_PLAYERS]][0].SubAICount = 0;
/*
...
void MakeCall()
{
   ...
   for (int c=0; c<ChCount; c++) {
	 ...
	 if (DinoInfo[AI_to_CIndex[TargetCall] ][cptr->SUBAI].DangerCall)
		 if (cptr->AI<10) {
			 cptr->State=2;
...
ENDTRACE:
  ...
  //-> You hit a dino: Add visual effects...
  Weapon.shakel = 10; //Generates crosshair pulse
...
//======= character damage =========//
  ...
  if (mort) {
	  Characters[ShotDino].Health = 0;
  } else {
	  if (!ANIMAL_ATTACK_MODE) {
		Characters[ShotDino].Health-=(int)WeapInfo[CurrentWeapon].Power;
	  } else {
		  Characters[ShotDino].Health-=(int)WeapInfo[CurrentWeapon].Power*3; //Extra power when in this mode
	  }
  }
   ...
   if (!Characters[ShotDino].Health) {	 	 
	 ...
	 if (Characters[ShotDino].AI<10) 
		...
		if (!CHEAT_NOFEAR) {
			...
			Characters[ShotDino].last_saw_player_time = 0;
			Characters[ShotDino].last_saw_player_pos.x = PlayerX;
			Characters[ShotDino].last_saw_player_pos.z = PlayerZ;
...
*/
void MakeShotNew(float ax, float ay, float az,
              float bx, float by, float bz)
{
  int sres;  
  Vector3d waterhl;
...
ENDTRACE:
  ...
  if (hitwater == true) {
	  AddElements(waterhl.x, waterhl.y, waterhl.z, partWater, (int)(4 + WeapInfo[CurrentWeapon].Power*3));
	  //AddElements(bx, GetLandH(bx, bz), bz, partBubble);  
	  AddWCircle(waterhl.x, waterhl.z, 1.2f);
	  AddWCircle(waterhl.x, waterhl.z, 1.2f);
  }
...
{  
  ...
  SetAmbient3d(SShipModel.SoundFX[0].length, 
			   ...
			   SupplyShip.pos.x, SupplyShip.pos.y, SupplyShip.pos.z);
  ...
  if (Ship.State==2)
   ...
   if (L<4000)
	if (VectorLength(SubVectors(PlayerPos, SupplyShip.pos))<(ctViewR+2)*256) {
        ...
        SupplyShip.tgpos.y = max(SupplyShip.tgpos.y, GetLandUpH(Ship.pos.x, Ship.pos.z) + 2048);
	}
//==============================//
...
//======= rotation ============//
  ...
  if (fabs(drspd) < fabs(rspd)) { SupplyShip.alpha = SupplyShip.tgalpha; SupplyShip.rspeed/=2; }
...
void AnimateAmmoBag()
{
    ...
    if (l<256) {
      AddVoice3d(AmmoModel.SoundFX[1].length, AmmoModel.SoundFX[1].lpData,
    ...
    if (AmmoBag.pos.y < h) {
        ...
        AmmoBag.State = 2;
        AmmoBag.FTime = 1;
        AddVoice3d(AmmoModel.SoundFX[0].length, AmmoModel.SoundFX[0].lpData,
		           AmmoBag.pos.x, AmmoBag.pos.y, AmmoBag.pos.z);   
...
{	
	...
	//fix subai
	PrintLog(DinoInfo[TrophyRoom.Body[TrophyBody].ctype][0].Name);
	PrintLog("\n");
