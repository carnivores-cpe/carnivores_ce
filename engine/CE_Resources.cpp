                  ...
                  DWORD dwBytes)
{
   ...
   LPVOID res = HeapAlloc(GetProcessHeap(), 
                          ...
                          dwBytes);
	if (!res) {
		wsprintf(logt, "[Heap] Error: %x Flags: %x, Size: %x",GetLastError(), dwFlags,dwBytes );
		DoHalt(logt);
	}
...
void LoadPicture(TPicture &pic, LPSTR pname)
{
	...
	//pic.lpImage = new WORD[pic.W * pic.H * 2];
...
#else
  mptr->lpTexture2 = new WORD[(1+th)*128*2];
#endif
...
#else
  mptr->lpTexture3 = new WORD[(1+th)*64*2];
#endif
...
void LoadResources()
{
    ...
    ReadFile(hfile, &tc, sizeof(int), &l, NULL);
    ReadFile(hfile, &mc, sizeof(int), &l, NULL);

	
	ReadFile(hfile,  FadeRGB, sizeof(int)*3*3, &l, NULL); 
	ReadFile(hfile, TransRGB, sizeof(int)*3*3, &l, NULL); 
	...
	wsprintf(logt,"Loading %d Map Textures ->", tc);
    PrintLog(logt);
	...
	int FgCount;
	ReadFile(hfile, &FgCount, sizeof(int), &l, NULL); 
	...
	ReadFile(hfile, &RdCount, sizeof(int), &l, NULL); 
	...
	ReadFile(hfile, &AmbCount, sizeof(int), &l, NULL); 
...
/////////////////
	...
	ReadFile(hfile, &WtrCount, sizeof(int), &l, NULL); 	
...
void LoadCharacters()
{
	...
	for (int c=0; c<ChCount; c++) {
		...
		//Characters[c].FTime = 100;
		//Characters[c].Phase = 2;
	}
	for (int c=0; c<TotalC; c++) if (pres[c]) {
		if (!ChInfo[c][0].mptr) { //Handle sub AI too
			wsprintf(logt, "HUNTDAT\\%s", DinoInfo[c][0].FName);
			PrintLog("Loading: ");	PrintLog(logt);	PrintLog("\n");
			LoadCharacterInfo(ChInfo[c][0], logt);

			//PrintLog("Morphing Character... \n");
			//CreateMorphedModelEditor(ChInfo[c][0].mptr, &ChInfo[c][0].Animation[3], ChInfo[c][0].Animation[3].AniTime/2, 1.0);
			//PrintLog("Done\n");

			//-> Load subais
			for (int si = 0; si<DinoInfo[c][0].SubAICount;si++) {
				if (strlen(DinoInfo[c][si+1].FName) > 0) {
					wsprintf(logt, "HUNTDAT\\MODS\\%s", DinoInfo[c][si+1].FName);
					PrintLog("Loading Mod: "); PrintLog(logt);PrintLog("\n");
					LoadCharacterInfo(ChInfo[c][si+1], logt);
				} else {
					//-> No file... Just load the skin replacement
					//-> Move this to a single array of loaded models files, and have each model contain a mem reference (&) to a loaded model file.
					//-> Head free error generated through LoadPictureTGA...
					wsprintf(logt, "HUNTDAT\\%s", DinoInfo[c][0].FName);
					PrintLog("Loading Mod Override for: "); PrintLog(logt);
					LoadCharacterInfo(ChInfo[c][si+1], logt);
					wsprintf(logt,"HUNTDAT\\MODS\\%s",DinoInfo[c][si+1].Skin);
					LoadPictureTGA(tmppic,logt);

					_HeapFree(Heap, 0, ChInfo[c][si+1].mptr->lpTexture);
					ChInfo[c][si+1].mptr->lpTexture = (WORD*) _HeapAlloc(Heap, 0, ChInfo[c][si+1].mptr->TextureSize);  

					PrintLog(" using TGA ");PrintLog(logt);PrintLog("\n");
					memcpy(ChInfo[c][si+1].mptr->lpTexture,tmppic.lpImage,ChInfo[c][si+1].mptr->TextureSize);
					BrightenTexture(ChInfo[c][si+1].mptr->lpTexture, ChInfo[c][si+1].mptr->TextureSize/2);
					DATASHIFT(ChInfo[c][si+1].mptr->lpTexture, ChInfo[c][si+1].mptr->TextureSize);
					GenerateModelMipMaps(ChInfo[c][si+1].mptr);
					GenerateAlphaFlags(ChInfo[c][si+1].mptr);
	...
	for (int c=10; c<21; c++)
#endif
		if (TargetDino & (1<<c) && c != AI_PLAYERS) 	
			if (!DinoInfo[AI_to_CIndex[c]][0].CallIcon.lpImage) {		
			  wsprintf(logt, "HUNTDAT\\MENU\\PICS\\call%d.tga", c-9);
	          LoadPictureTGA(DinoInfo[AI_to_CIndex[c]][0].CallIcon, logt); 
			  conv_pic(DinoInfo[AI_to_CIndex[c]][0].CallIcon);
			}
	...
	for (int c=0; c<TotalW; c++) 
		if (WeaponPres & (1<<c)) 	{			
			...
			if (!Weapon.BulletPic[c].lpImage) {
			 ...
			 //ApplyAlphaFlags(Weapon.BulletPic[c].lpImage, Weapon.BulletPic[c].W*Weapon.BulletPic[c].W);
			 PrintLog("Loading: ");  PrintLog(logt);  PrintLog("\n");
			}
...
#else
	chinfo.mptr->lpTexture = new WORD[ts];
#endif
...
#else
	chinfo.Animation[a].aniData = new short int[(chinfo.mptr->VCount*chinfo.Animation[a].FramesCount*6)];
#endif
...
#else
	chinfo.SoundFX[s].lpData = new short int[chinfo.SoundFX[s].length];
#endif
...
void SaveScreenShot()
 { 
  ...
  //MessageBeep(0xFFFFFFFF);
    ...
    CloseHandle(hf);
	wsprintf(logt, "Screenshot saved as %s", t);
...
void ReadMResMods(FILE* stream) 
{
	//== Adelphospro ==//
	//-> Read Mod file - branch mods, and load data within
	int newSUBAI;//Amount of SUBAIs
	int parentAI; //Parent AI of mod
	int runcnt,walkcnt,slidecnt,swimcnt,idlecnt,attackcnt,diecnt,eatcnt,sleepcnt; //Animation counters
char line[256], *value;
    while (fgets( line, 255, stream)) 
	{
		if (strstr(line, "}")) break;
		if (strstr(line, "{")) {
			...
			while (fgets( line, 255, stream)) {				
				...
				if (strstr(line, "mass"     )) DinoInfo[parentAI][newSUBAI].Mass      = (float)atof(value);
				if (strstr(line, "length"   )) DinoInfo[parentAI][newSUBAI].Length    = (float)atof(value);
				if (strstr(line, "radius"   )) DinoInfo[parentAI][newSUBAI].Radius    = (float)atof(value);
				if (strstr(line, "health"   )) DinoInfo[parentAI][newSUBAI].Health0   = atoi(value);
				if (strstr(line, "basescore")) DinoInfo[parentAI][newSUBAI].BaseScore = atoi(value);
				if (strstr(line, "ai"       )) {
					//DinoInfo[TotalC][0].AI        = atoi(value);
					//-> Register new mod
					parentAI = AI_to_CIndex[atoi(value)];
					DinoInfo[parentAI][0].SubAICount++;
					newSUBAI = DinoInfo[parentAI][0].SubAICount; 
					strcpy(DinoInfo[parentAI][newSUBAI].FName,"");
					DinoInfo[parentAI][newSUBAI].SwimSpeed = 0;
					DinoInfo[parentAI][newSUBAI].RunSpeed = 0;
					DinoInfo[parentAI][newSUBAI].JumpSpeed = 0;
					DinoInfo[parentAI][newSUBAI].WalkSpeed = 0;
					DinoInfo[parentAI][newSUBAI].Fear = 0;
					wsprintf(logt,"		+Added 1. Total SubAI: %d. CIndex:%d, Name of ParentAI:%s\n",newSUBAI,parentAI,DinoInfo[parentAI][0].Name);
					PrintLog(logt);
				}
				if (strstr(line, "fear"     )) DinoInfo[parentAI][newSUBAI].Fear        = (float)atoi(value);
				if (strstr(line, "runspeed" )) DinoInfo[parentAI][newSUBAI].RunSpeed        = (float)atoi(value);
				if (strstr(line, "walkspeed" )) DinoInfo[parentAI][newSUBAI].WalkSpeed        = (float)atoi(value);
				if (strstr(line, "jumpspeed" )) DinoInfo[parentAI][newSUBAI].JumpSpeed        = (float)atoi(value);
				if (strstr(line, "swimspeed" )) DinoInfo[parentAI][newSUBAI].SwimSpeed        = (float)atoi(value);
				if (strstr(line, "animation.run")) {
					ChInfo[parentAI][newSUBAI].RUN[runcnt] = atoi(value);
					runcnt++;
				}
				if (strstr(line, "animation.walk")) {
					ChInfo[parentAI][newSUBAI].WALK[walkcnt] = atoi(value);
					walkcnt++;
				}
				if (strstr(line, "animation.swim")) {
					ChInfo[parentAI][newSUBAI].SWIM[swimcnt] = atoi(value);
					swimcnt++;
				}
				if (strstr(line, "animation.idle")) {
					ChInfo[parentAI][newSUBAI].IDLE[idlecnt] = atoi(value);
					idlecnt++;
				}
				if (strstr(line, "animation.attack")) {
					ChInfo[parentAI][newSUBAI].ATTACK[attackcnt] = atoi(value);
					attackcnt++;
				}
				if (strstr(line, "animation.die")) {
					ChInfo[parentAI][newSUBAI].DIE[diecnt] = atoi(value);
					diecnt++;
				}
				if (strstr(line, "animation.eat")) {
					ChInfo[parentAI][newSUBAI].EAT[eatcnt] = atoi(value);
					eatcnt++;
				}
				if (strstr(line, "animation.sleep")) {
					ChInfo[parentAI][newSUBAI].SLEEP[sleepcnt] = atoi(value);
					sleepcnt++;
				}

				if (strstr(line, "smell"    )) DinoInfo[parentAI][newSUBAI].SmellK    = (float)atof(value);
				if (strstr(line, "hear"     )) DinoInfo[parentAI][newSUBAI].HearK     = (float)atof(value);
				if (strstr(line, "look"     )) DinoInfo[parentAI][newSUBAI].LookK     = (float)atof(value);
				if (strstr(line, "shipdelta")) DinoInfo[parentAI][newSUBAI].ShDelta   = (float)atof(value);
				if (strstr(line, "scale0"   )) DinoInfo[parentAI][newSUBAI].Scale0    = atoi(value);
				if (strstr(line, "scaleA"   )) DinoInfo[parentAI][newSUBAI].ScaleA    = atoi(value);
				if (strstr(line, "danger"   )) DinoInfo[parentAI][newSUBAI].DangerCall= TRUE;

				if (strstr(line, "name")) {					
					value = strstr(line, "'"); if (!value) DoHalt("Script loading error");
					value[strlen(value)-2] = 0;
					strcpy(DinoInfo[parentAI][newSUBAI].Name, &value[1]); }

				if (strstr(line, "file")) {					
					value = strstr(line, "'"); if (!value) DoHalt("Script loading error");
					value[strlen(value)-2] = 0;
					strcpy(DinoInfo[parentAI][newSUBAI].FName, &value[1]);}

				if (strstr(line, "skin")) {					
					value = strstr(line, "'"); if (!value) DoHalt("Script loading error");
					value[strlen(value)-2] = 0;
					strcpy(DinoInfo[parentAI][newSUBAI].Skin, &value[1]);}

				if (strstr(line, "pic")) {					
					value = strstr(line, "'"); if (!value) DoHalt("Script loading error");
					value[strlen(value)-2] = 0;
					strcpy(DinoInfo[parentAI][newSUBAI].PName, &value[1]);}
			}
		}
...
void ReadCharacters(FILE *stream)
{
    ...
    while (fgets( line, 255, stream)) 
	{
		...
		if (strstr(line, "{")) {
					...
					eatcnt = 0;
					sleepcnt = 0;
					DinoInfo[TotalC][0].SwimSpeed = 0;
					DinoInfo[TotalC][0].RunSpeed = 0;
					DinoInfo[TotalC][0].JumpSpeed = 0;
					DinoInfo[TotalC][0].WalkSpeed = 0;
					DinoInfo[TotalC][0].Fear = 1;
			while (fgets( line, 255, stream)) {		
				
				if (strstr(line, "}")) { 
                    AI_to_CIndex[DinoInfo[TotalC][0].AI] = TotalC;
					TotalC++; 
					break; 
				}
				...
				if (strstr(line, "mass"     )) DinoInfo[TotalC][0].Mass      = (float)atof(value);
				if (strstr(line, "length"   )) DinoInfo[TotalC][0].Length    = (float)atof(value);
				if (strstr(line, "radius"   )) DinoInfo[TotalC][0].Radius    = (float)atof(value);
				if (strstr(line, "health"   )) DinoInfo[TotalC][0].Health0   = atoi(value);
				if (strstr(line, "basescore")) DinoInfo[TotalC][0].BaseScore = atoi(value);
				if (strstr(line, "ai"       )) DinoInfo[TotalC][0].AI        = atoi(value);
				if (strstr(line, "fear"     )) DinoInfo[TotalC][0].Fear        = (float)atoi(value);
				if (strstr(line, "runspeed" )) DinoInfo[TotalC][0].RunSpeed        = (float)atoi(value);
				if (strstr(line, "walkspeed" )) DinoInfo[TotalC][0].WalkSpeed        = (float)atoi(value);
				if (strstr(line, "jumpspeed" )) DinoInfo[TotalC][0].JumpSpeed        = (float)atoi(value);
				if (strstr(line, "swimspeed" )) DinoInfo[TotalC][0].SwimSpeed        = (float)atoi(value);
				if (strstr(line, "animation.run")) {
					ChInfo[TotalC][0].RUN[runcnt] = atoi(value);
					runcnt++;
				}
				if (strstr(line, "animation.walk")) {
					ChInfo[TotalC][0].WALK[walkcnt] = atoi(value);
					walkcnt++;
				}
				if (strstr(line, "animation.swim")) {
					ChInfo[TotalC][0].SWIM[swimcnt] = atoi(value);
					swimcnt++;
				}
				if (strstr(line, "animation.idle")) {
					ChInfo[TotalC][0].IDLE[idlecnt] = atoi(value);
					idlecnt++;
				}
				if (strstr(line, "animation.attack")) {
					ChInfo[TotalC][0].ATTACK[attackcnt] = atoi(value);
					attackcnt++;
				}
				if (strstr(line, "animation.die")) {
					ChInfo[TotalC][0].DIE[diecnt] = atoi(value);
					diecnt++;
				}
				if (strstr(line, "animation.eat")) {
					ChInfo[TotalC][0].EAT[eatcnt] = atoi(value);
					eatcnt++;
				}
				if (strstr(line, "animation.sleep")) {
					ChInfo[TotalC][0].SLEEP[sleepcnt] = atoi(value);
					sleepcnt++;
				}
				if (strstr(line, "smell"    )) DinoInfo[TotalC][0].SmellK    = (float)atof(value);
				if (strstr(line, "hear"     )) DinoInfo[TotalC][0].HearK     = (float)atof(value);
				if (strstr(line, "look"     )) DinoInfo[TotalC][0].LookK     = (float)atof(value);
				if (strstr(line, "shipdelta")) DinoInfo[TotalC][0].ShDelta   = (float)atof(value);
				if (strstr(line, "scale0"   )) DinoInfo[TotalC][0].Scale0    = atoi(value);
				if (strstr(line, "scaleA"   )) DinoInfo[TotalC][0].ScaleA    = atoi(value);
				if (strstr(line, "danger"   )) DinoInfo[TotalC][0].DangerCall= TRUE;
				if (strstr(line, "name")) {					
					value = strstr(line, "'"); if (!value) DoHalt("Script loading error");
					value[strlen(value)-2] = 0;
					strcpy(DinoInfo[TotalC][0].Name, &value[1]); }

				if (strstr(line, "file")) {					
					value = strstr(line, "'"); if (!value) DoHalt("Script loading error");
					value[strlen(value)-2] = 0;
					strcpy(DinoInfo[TotalC][0].FName, &value[1]);}

				if (strstr(line, "pic")) {					
					value = strstr(line, "'"); if (!value) DoHalt("Script loading error");
					value[strlen(value)-2] = 0;
					strcpy(DinoInfo[TotalC][0].PName, &value[1]);}
			}
		}
...
void LoadResourcesScript()
{
	...
	//-> Now, read each mes file and load SubAIs...
   	WIN32_FIND_DATA fd;
	HANDLE hFind = FindFirstFile("huntdat\\mods\\*.mres" , &fd);
	if( hFind != INVALID_HANDLE_VALUE)
		{
			do {
				wsprintf(fname,"huntdat\\mods\\%s",fd.cFileName);
				PrintLog("Reading Mod ");PrintLog(fname); PrintLog("...\n");
				stream = fopen(fname,"r");
				if (!stream) {
					wsprintf(logt,"ERROR - Cannot read %s\n",fname);
					DoHalt(logt);
				}
				while (fgets( line, 255, stream)) {
					 if (line[0] == '.') break;
					 //-> Is it a dinosaur mod?
						PrintLog("	+ Reading Dinosaur...\n");
						if (strstr(line, "mods") ) ReadMResMods(stream);
					//-> Weapon?
					//-> Done
					}
					fclose (stream);
...
#ifdef _soft
	PrintLog("Carnivores 2 ADP Soft video driver.");
#endif
	// PrintLog(" Build v2.13. Jun.26 2002.\n");
	PrintLog(" Build ADPv2.14.112.4. " __DATE__ "\n"); // alacn
}
