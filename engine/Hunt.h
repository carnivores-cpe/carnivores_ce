...
typedef struct _TCharacter  {
  ...
  int PPMorphTime, PrevPhase,PrevPFTime, Phase, FTime, SetPhase;
  ...
  Vector3d pos, rpos;
  Vector3d last_saw_player_pos;
  int last_saw_player_time;
...
public:
	...
	TVehicle(void);
	~TVehicle(void);
	static void process(TCharacter *cptr);
	static void process_slide();	
	static void process_movement();
...
typedef struct _LocalMPlayerData {
	//Store some local information concerning players, such as their ChCount ID
	int ChCount;
	int NetID; //Network ID of the player
	bool active; //Only draw if the player is active
} MPlayerLocalData;

typedef struct _TDinoInfo {
	char Name[48], FName[48], PName[48],Skin[48];	
	int Health0, AI,SubAICount;
	float RunSpeed, WalkSpeed, SwimSpeed, JumpSpeed, Fear;
	BOOL DangerCall;
...
_EXTORNOT   char logt[128],TypedText[128], USE_MESSAGE[128];
_EXTORNOT	int TypedTextLength;
void CreateLog();
...
_EXTORNOT   TMessageList MessageList[32],MessageListDelayed[128], CommandLog[512]; //Max of 32 or so on screen at one time
_EXTORNOT   char    ProjectName[128];
_EXTORNOT   int     _GameState,MessageListCnt,MessageListDelayedCnt, CommandLogCnt, CommandLogIndex;
_EXTORNOT   TSFX    fxCall[15][3], fxScream[4], TypeSound[4];
...
_EXTORNOT	TMpModel	PlayerModel;
...
//========= GAME ====================//
_EXTORNOT int TargetDino, CurDino, TargetArea, TargetWeapon, WeaponPres, TargetCall,
...
_EXTORNOT BOOL ScentMode, CamoMode, 
			   ...
			   TrophyMode, DoubleAmmo, Supply, SupplyUsed,Noah,TransSoftWater, LooseMouse;

_EXTORNOT TTrophyRoom TrophyRoom;
//_EXTORNOT TPlayerR PlayerR[16];
_EXTORNOT TPicture LandPic,DinoPic,DinoPicM, MapPic, WepPic, ConsolePic;
...
//============= Characters ==============//
_EXTORNOT TPicture  PausePic, ExitPic, TrophyExit, TrophyPic,menuback;
_EXTORNOT TModel *SunModel;
_EXTORNOT TCharacterInfo WCircleModel;
_EXTORNOT TModel *CompasModel;
_EXTORNOT TModel *Binocular;
_EXTORNOT TDinoInfo DinoInfo[64][256]; //Upto 256 characters AI. This holds script data
...
_EXTORNOT int ChCount, WCCount, ElCount, SnCount,
              ShotDino, TrophyBody, ServerCh, MPlayersCount, human_model_ch;
...
_EXTORNOT	MPlayerLocalData	MPLocalData[200];

_EXTORNOT   int     PrevTime, TimeDt, T, Takt, RealTime, StepTime, MeleeTime, LastMeleeTime, MyHealth, ExitTime, PlayerPainTime,
                    ChCallTime, CallLockTime, NextCall,MyEnergy,LastStepSoundTime; //ADP, 4.19.09, added MyEnergy; ADP 7.29.09 added MeleeTime for melee tests
_EXTORNOT   float   DeltaT;
_EXTORNOT   float   CameraX, CameraY, CameraZ, CameraAlpha, CameraBeta;
_EXTORNOT   float   PlayerX, PlayerY, PlayerZ, PlayerAlpha, PlayerBeta,PainAlpha, 
...
#define AI_PLAYERS    29
#define AI_BODY		  30
#define AI_LANDJEEP	  50
...
_EXTORNOT BOOL WATERANI,Clouds,SKY,GOURAUD,
               ...
               SNOW, SWIM, PAUSE, OPTICMODE, BINMODE, EXITMODE, MapMode, RunMode, CrouchMode, SprintMode, USE_MELEE, IN_VEHICLE; //-> 4.13.09, ADP, Added SprintMode
// -> Cheats
_EXTORNOT BOOL FLY, CHEAT_GOD,CHEAT_FASTWALK,CHEAT_NOFEAR,CHEAT_FREEZE,CHEAT_FREEAMMO, CHEAT_ENABLED, POTHEAD;

// -> Fin tests
_EXTORNOT BOOL ANIMAL_ATTACK_MODE; //Causes ALL animals to move towards the hunter with no fear

// -> Networking
_EXTORNOT bool IsServer, NetworkEnabled;
void InitNetwork();
void DestroyNetwork();
