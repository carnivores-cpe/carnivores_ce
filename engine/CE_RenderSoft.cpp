...
void ShowControlElements()
{
  ...
  int lastYMSG = 10;
  if (Weapon.state)
	  lastYMSG = (Weapon.BulletPic[CurrentWeapon].H * int(DoubleAmmo+1)) + 10;
  ...
  if (TypingMode) {
	  ...
	  //STTextOut((WinW/2)-(GetTextW(hdcMain, TypedText)/2), WinH/2, logt, RGB(128,255,128));
	  //Draw bg bitmap
	  int times = WinW / 32;
	  for (int times = 0; times < (WinW / 32); times++) {
		DrawPicture(times * 32, 0, ConsolePic);
	  }
	  //Draw command log
	  int CommandLogIndexDynamic = 0;
	  if (CommandLogIndex == -1) {
		CommandLogIndexDynamic = max(CommandLogCnt - ((256-16)/16),0);
	  } else {
		CommandLogIndexDynamic = CommandLogIndex;
	  }
	  for (int cmdlg = CommandLogIndexDynamic; cmdlg < CommandLogCnt && 16*(cmdlg-CommandLogIndexDynamic) < 256-16; cmdlg++) {
		  STTextOut(5, 16*(cmdlg-CommandLogIndexDynamic), CommandLog[cmdlg].mtext, RGB(91,91,91));
	  }
	  //Draw text
	  STTextOut(5, 256 - 16, logt, RGB(128,255,128));
  }

  if (DEBUG_ASKFORPASS) {
	  wsprintf(logt,"- Enter Password Now and Press Enter -");
  ...
  if (strlen(USE_MESSAGE) > 0)
	  STTextOut(VideoCX - GetTextW(hdcMain, USE_MESSAGE)/2, WinH / 3, USE_MESSAGE, RGB(128,255,128));
...
//=========== player model ========//
   ...
   //PlayerModel.rpos.x = PlayerX;
   //PlayerModel.rpos.y = PlayerY;
   //PlayerModel.rpos.z = PlayerZ;

   // int i = ChRenderList[0].ICount++;
   // ChRenderList[0].Items[i].CType = 6;
...
void CE_RenderC2Character(C2Character* character);

void CE_RenderManagedCharacters()
{
	// show time
	int numManagedCharacters = ManagedCharacters.size();

	for (int c = 0; c < numManagedCharacters; c++) {
		CE_RenderC2Character(ManagedCharacters.at(c).get());
	}
}

#include "modern_src\C2Character.h"
#include "modern_src\C2Geometry.h"

void CE_RenderC2Character(C2Character* character)
{
	Vector3d rpos;
	rpos.x = character->getCurrentPosition().x - CameraX;
	rpos.y = character->getCurrentPosition().y - CameraY;
	rpos.z = character->getCurrentPosition().z - CameraZ;

	rpos = RotateVector(rpos);

	float zs = (float)VectorLength(rpos);
	...
	TModel* mMptr = mGeo->getTModel();

	float wh = GetLandUpH(character->getCurrentPosition().x, character->getCurrentPosition().z);
	waterclip = FALSE;

	if (!UNDERWATER)
		if (wh > character->getCurrentPosition().y + 32 * 2) {
			waterclipbase.x = character->getCurrentPosition().x - CameraX;
			waterclipbase.y = wh - CameraY;
			waterclipbase.z = character->getCurrentPosition().z - CameraZ;
			waterclipbase = RotateVector(waterclipbase);
			waterclip = TRUE;
		}

	if (fabs(rpos.z) + fabs(rpos.x) <2560)
		RenderModelClip(mMptr,
		rpos.x, rpos.y, rpos.z, 240, 0,
		-character->getCurrentAlpha() + pi / 2 + CameraAlpha,
		CameraBeta);
	else
		if (waterclip)
			RenderModelClipWater(mMptr,
			rpos.x,rpos.y, rpos.z, 240, 0,
			-character->getCurrentAlpha() + pi / 2 + CameraAlpha,
			CameraBeta);
		else
...
void RenderPlayerModel()
{
	TCharacter *cptr = &Characters[human_model_ch];

	cptr->pos.x = PlayerModel.rpos.x;
	cptr->pos.y = PlayerModel.rpos.y;
...
void RenderShip()
{
   ...
   CreateMorphedModel(ShipModel.mptr, &ShipModel.Animation[0], Ship.FTime, 2.0);
...
void DrawHMap()
{
   //if (WinH < 280) return; 
	if (!Weapon.state)
		DrawPicture((WinW-MapPic.W/2)-MapPic.W/2, 128 - MapPic.H/2, MapPic);	
   int xx = (WinW-MapPic.W/2) - 128 + (CCX>>2);
   int yy = 128 - 128 + (CCY>>2)+6;
  ...
  for (int c=0; c<ChCount; c++) {
   ...
   if (! (TargetDino & (1<<Characters[c].AI)) && Characters[c].AI != AI_PLAYERS ) continue;
   if (!Characters[c].Health) continue;
   xx = (WinW-MapPic.W/2) - 128 + (int)Characters[c].pos.x / 1024;
   yy = 128 - 128 + (int)Characters[c].pos.z / 1024;
   if (yy<=0 || yy>=WinH) continue;
   if (xx<=0 || xx>=WinW) continue;
   int color_code = 30;
   int color_index = 10;
   if (Characters[c].AI == AI_PLAYERS) {
		color_code = 31;
		color_index = 5;
   }
   *((WORD*)lpVideoBuf + yy*1024 + xx) = color_code<<color_index;   
   *((WORD*)lpVideoBuf + yy*1024 + xx+1) = color_code<<color_index;  
   *((WORD*)lpVideoBuf + yy*1024 + xx+2) = color_code<<color_index;    
   yy++;
   *((WORD*)lpVideoBuf + yy*1024 + xx) = color_code<<color_index;    
   *((WORD*)lpVideoBuf + yy*1024 + xx+1) = color_code<<color_index;    
   *((WORD*)lpVideoBuf + yy*1024 + xx+2) = color_code<<color_index;    
...
/*
    ...
    STTextOut(x, y0   , DinoInfo[dtype][subai].Name, 0x0000BFBF);    
...
void Render_LifeInfo(int li)
{
    ...
    STTextOut(x, y, DinoInfo[ctype][subai].Name, 0x0000b000);    
...
void ShowVideo()
{
 ...
 if (PlayerPainTime > 0)
   for (int y=0; y<WinH; y++) 
    for (int x=0; x<WinW; x++)
     *((WORD*)lpVideoBuf + y*1024 + x) &= HiColor(45, 0, 0);

 /* Pothead */
  if (POTHEAD)
   for (int y=0; y<WinH; y++) 
    for (int x=0; x<WinW; x++)
     *((WORD*)lpVideoBuf + y*1024 + x) &= HiColor(0, 255, 255);
 /* Flashlight effect test */
   //for (int y=0; y<WinH; y++) 
	  // for (int x=0; x<WinW; x++) {
		 //  if (y < ((WinH/2)+128) && y > ((WinH/2)-128) && x > ((WinW/2)-128) && x < ((WinW/2)+128))
			//	*((WORD*)lpVideoBuf + y*1024 + x) = FadeTab[64][*((WORD*)lpVideoBuf + y*1024 + x)];
			////*((WORD*)lpVideoBuf + y*1024 + x) == HiColor(R, G, B)
	  // }
