...
void CheckCollision(float &cx, float &cz)
{
   ...
   if (!TrophyMode) return; //Animal Body collision.. Rework this to allow human resistance to smaller dinosaurs
   for (int c=0; c<ChCount; c++) {
	   float ph = max(Characters[c].pos.y,PlayerY) - min (Characters[c].pos.y, PlayerY); //Player Height difference added by Adelphospro
	   float px = Characters[c].pos.x;
	   float pz = Characters[c].pos.z;
	   float CR = DinoInfo[ Characters[c].CType ][Characters[c].SUBAI].Radius / 2;
