...
void d3dCatchError(char sourcefunc[128], HRESULT result);
...
HRESULT CreateDevice(DWORD dwWidth, DWORD dwHeight)
{
	...
	if (FAILED(hRes)) d3dCatchError("CreateBackBuffer", hRes);
	...
	if (FAILED(hRes)) d3dCatchError("QueryInterface", hRes);
...
void d3dCatchError(char sourcefunc[128], HRESULT result) {
	PrintLog("-----D3D_ERROR-----\n");
	char d3dErrorText[128], d3derrorhead[128];
	wsprintf(d3derrorhead, "%s-> ", sourcefunc);
	//-> Just in case one of the 'cases' is missing a discription:
	wsprintf(d3dErrorText, "%sOne of the d3d errors!! Details undefined", d3derrorhead);

	switch (result) {
	case DDERR_INVALIDOBJECT:
		wsprintf(d3dErrorText, "%sInvalid Object", d3derrorhead);
		break;
	case DDERR_INVALIDPARAMS:
		wsprintf(d3dErrorText, "%sInvalid Params", d3derrorhead);
		break;
	case DDERR_OUTOFMEMORY:
		wsprintf(d3dErrorText, "%sOut of Memory", d3derrorhead);
		break;
	case D3DERR_BADMAJORVERSION:
		wsprintf(d3dErrorText, "%sBad Major Version", d3derrorhead);
		break;
	case D3DERR_BADMINORVERSION:
		wsprintf(d3dErrorText, "%sBad Minor Version", d3derrorhead);
		break;
	case D3DERR_EXECUTE_CLIPPED_FAILED:
		wsprintf(d3dErrorText, "%sExec Clipped Failed", d3derrorhead);
		break;
	case D3DERR_EXECUTE_CREATE_FAILED:
		wsprintf(d3dErrorText, "%sExecute Create Failed", d3derrorhead);
		break;
	case D3DERR_EXECUTE_DESTROY_FAILED:
		wsprintf(d3dErrorText, "%sExec Destory failed", d3derrorhead);
		break;
	case D3DERR_EXECUTE_FAILED:
		wsprintf(d3dErrorText, "%sExec Failed", d3derrorhead);
		break;
	case D3DERR_EXECUTE_LOCK_FAILED:
		wsprintf(d3dErrorText, "%sExec Lock Failed", d3derrorhead);
		break;
	case D3DERR_EXECUTE_LOCKED:
		wsprintf(d3dErrorText, "%sExec Locked", d3derrorhead);
		break;
	case D3DERR_EXECUTE_NOT_LOCKED:
		wsprintf(d3dErrorText, "%sExec Not Locked", d3derrorhead);
		break;
	case D3DERR_EXECUTE_UNLOCK_FAILED:
		wsprintf(d3dErrorText, "%sExec Unlock Failed", d3derrorhead);
		break;
	case D3DERR_INITFAILED:
		wsprintf(d3dErrorText, "%sInit Failed", d3derrorhead);
		break;
	case D3DERR_INBEGIN:
		wsprintf(d3dErrorText, "%sIn Begin(?)", d3derrorhead);
		break;
	case D3DERR_INVALID_DEVICE:
		wsprintf(d3dErrorText, "%sInvalid Device", d3derrorhead);
		break;
	case D3DERR_INVALIDCURRENTVIEWPORT:
		wsprintf(d3dErrorText, "%sInvalid Current Viewport", d3derrorhead);
		break;
	case D3DERR_INVALIDPALETTE:
		wsprintf(d3dErrorText, "%sInvalid Palette", d3derrorhead);
		break;
	case D3DERR_INVALIDPRIMITIVETYPE:
		wsprintf(d3dErrorText, "%sInvalid Primitive Type", d3derrorhead);
		break;
	...
	case D3DERR_SURFACENOTINVIDMEM:
		wsprintf(d3dErrorText, "%sSurface not in Video Memory", d3derrorhead);
		break;
	...
	case D3DERR_ZBUFF_NEEDS_SYSTEMMEMORY:
		wsprintf(d3dErrorText, "%sZBuffer needs system memory", d3derrorhead);
		break;
	case D3DERR_ZBUFF_NEEDS_VIDEOMEMORY:
		wsprintf(d3dErrorText, "%szBuffer needs video memory", d3derrorhead);
		break;
	default:
		wsprintf(d3dErrorText, "%sUnknown Error: %X", d3derrorhead, result);
		break;
	}
	PrintLog(d3dErrorText);
	DoHalt(d3dErrorText);
}
...
void d3dDetectCaps()
{
	...
	if (VMFORMAT565)
		...
		if (!STARTCONV555) {
			...
			for (int c = 0; c<20; c++)
				if (DinoInfo[c][0].CallIcon.W)
					conv_pic555(DinoInfo[c][0].CallIcon);
...
// DX 10 use texture format A1R5G5B5
...
BOOL d3dAllocTexture(int i, int w, int h)
{
	...
	ddpf.dwGBitMask = 0x07e0;  //this is 555.... that's old. Newer machines use 565 (0x07E0) not 555 (0x3E0)
	...
	ddsd.ddsCaps.dwCaps = DDSCAPS_TEXTURE | DDSCAPS_VIDEOMEMORY; //Use systemmemory to fix d3d bug....

	hRes = lpDD->CreateSurface(&ddsd, &d3dMemMap[i].lpddTexture, NULL);
	if (FAILED(hRes)) {
		d3dMemMap[i].lpddTexture = NULL;
		// alacn ->
		//wsprintf(logt, "Error creating texture surface: %X. Perhaps no dedicated video memory or ran on laptop?\n", hRes);
		//PrintLog(logt);
		//DoHalt(logt);
		d3dCatchError("AllocTexture->CreateSurface", hRes);
		// <- alacn
		return FALSE;
	}
...
float GetSkyK(int x, int y)
{
	...
	k = (float)sqrt(float(SkySumR*SkySumR + SkySumG*SkySumG + SkySumB*SkySumB)) / 9;
...
void ProcessPain()
{
	if (PlayerPainTime == 0) return;
	RenderFSRect(RGB(45, 0, 0) + ((int)32 << 24));
}
...
void ProcessFade() {
	...
	if (ExitTime || RestartMode) {
		...
		if (FADING > 1 && (ExitTime && ExitTime < 1300 || RestartMode) && MyHealth) {
			...
			FADING -= 4;
			//small fi = clearer
			int fi = (50 * 4) - ((FADING * 4) / 2);
			RenderFSRect(RGB(0, 0, 0) + ((int)fi << 24));
		}
	...
	// ======= Fade In ======= //
	if (FADE_IN && FADING <= 40){
		FADING += 4;
		if (FADING > 65)
			FADING = 65;
		int fi = (50 * 4) - (FADING * 4);
		RenderFSRect(RGB(0, 0, 0) + ((int)fi << 24));
	}
	else if (FADE_IN) {
		FADING = 0;
...
void DrawTrophyText(int x0, int y0)
{
	...
	ddTextOut(x, y0, DinoInfo[dtype][subai].Name, 0x0000BFBF);

	x = x0;
	ddTextOut(x, y0 + 16, "Weight: ", 0x00BFBFBF);  x += GetTextW(hdcMain, "Weight: ");
	if (OptSys)
		sprintf(t, "%3.2ft ", DinoInfo[dtype][subai].Mass * scale * scale / 0.907);
	else
		sprintf(t, "%3.2fT ", DinoInfo[dtype][subai].Mass * scale * scale);

	ddTextOut(x, y0 + 16, t, 0x0000BFBF);    x += GetTextW(hdcMain, t);
	ddTextOut(x, y0 + 16, "Length: ", 0x00BFBFBF); x += GetTextW(hdcMain, "Length: ");

	if (OptSys)
		sprintf(t, "%3.2fft", DinoInfo[dtype][subai].Length * scale / 0.3);
	else
		sprintf(t, "%3.2fm", DinoInfo[dtype][subai].Length * scale);
...
void Render_LifeInfo(int li)
{
	...
	ddTextOut(x, y, DinoInfo[ctype][subai].Name, 0x0000b000);
	...
	*/
	...
	if (OptSys) sprintf(t, "Weight: %3.2ft ", DinoInfo[ctype][subai].Mass * Characters[li].scale * Characters[li].scale / 0.907);
	else        sprintf(t, "Weight: %3.2fT ", DinoInfo[ctype][subai].Mass * Characters[li].scale * Characters[li].scale);
	// <- alacn
...
void ShowControlElements()
{
	...
	if (TIMER) {
		...
		ddTextOut(WinEX - 81, 11, buf, RGB(128, 255, 128));    //Orig Color: 0x0020A0A0

		wsprintf(buf, "polys: %d", dFacesCount);
		ddTextOut(WinEX - 90, 24, buf, RGB(128, 255, 128));    //Orig Color: 0x0020A0A0
	}

	//-> First, clear message list of any expired messages
	...
	for (int msgID = 0; msgID < MessageListCnt; msgID++) {
		if (MessageList[msgID].timeleft) {
			if (RealTime>MessageList[msgID].timeleft) MessageList[msgID].timeleft = 0;
			ddTextOut(10, lastYMSG, MessageList[msgID].mtext, RGB(128, 255, 128));
			lastYMSG += 14;
		}
	}
	...
	if (TypingMode) {
		if (DEBUG_ASKFORPASS) {
			...
			if (int(strlen(TypedText)) > 0)
				wsprintf(logt, "-> %s", astCover);
			else
				wsprintf(logt, "->");
		}
		else {
			wsprintf(logt, "-> %s", TypedText);
		}
		ddTextOut((WinW / 2) - (GetTextW(hdcMain, TypedText) / 2), WinH / 2, logt, RGB(128, 255, 128));
	}

	if (DEBUG_ASKFORPASS) {
		wsprintf(logt, "Enter Password:");
		ddTextOut((WinW / 2) - (GetTextW(hdcMain, TypedText) / 2), WinH / 2 - 14, logt, RGB(180, 0, 0));
		if (!TypingMode)
			TypingMode = TRUE;
	}
	...
	if (ExitTime && ExitTime <= 4000) {
		...
		ddTextOut(VideoCX - GetTextW(hdcCMain, buf) / 2, y, buf, RGB(128, 255, 128));    //Orig Color: 0x0020A0A0
		wsprintf(buf, "%d seconds left.", 1 + ExitTime / 1000);
		ddTextOut(VideoCX - GetTextW(hdcCMain, buf) / 2, y + 18, buf, RGB(128, 255, 128));    //Orig Color: 0x0020A0A0
	}
	else if (ExitTime) {
		// -> Show 'Connecting to Base...'
		int y = WinH / 3;
		wsprintf(buf, "Connecting to Base... Please Stand By");
		ddTextOut(VideoCX - GetTextW(hdcMain, buf) / 2, y, buf, RGB(128, 255, 128));
	}
}
...
void DrawBox2(WORD *lfbPtr, int lsw, int xx, int yy, WORD c)
{
	yy = yy * lsw + xx;
	*(lfbPtr + yy) = c;
	*(lfbPtr + yy + 1) = c;
...
void DrawHMap()
{
	...
	DrawBox((WORD*)ddsd.lpSurface, lsw, xx + 1, yy + 1, 8 << GShift);
	...
	*/
	...
	if (RadarMode)
		for (c = 0; c<ChCount; c++) {
			...
			if (yy <= 0 || yy >= WinH) goto endmap;
			if (xx <= 0 || xx >= WinW) goto endmap;
			DrawBox2((WORD*)ddsd.lpSurface, lsw, xx - 1, yy - 1, 31 << RShift);
		}
...
void RenderEnergyBar()
{
	...
	for (int y = 1; y<3; y++) {
		...
		lpVertex->rhw = 1.f;
		lpVertex->color = RGB(0, 120, 0);// + (G<<8) + (R<<16);
		...
		lpVertex->sz = 0.99999f;
		lpVertex->rhw = 1.f;
		lpVertex->color = RGB(0, 180, 0);// + (G<<8) + (R<<16);
		lpVertex->specular = 0xFF000000;// + (G<<8) + (R<<16);
		lpVertex->tu = 0;
		lpVertex->tv = 0;
		lpVertex++;
...
{
	// == Modified by Adelphospro 4.14.09 == 
	float w = (float)LineSize;
	int gapsize = 0;
	if (DrawGap)
		gapsize = Weapon.shakel * 10;
	d3dStartBuffer();

	DWORD color = 0x80080808;
	for (int t = 0; t <= 1; t++) {
		if (t) {
			color = 0x8000AF10;
			sx -= 1; sy -= 1;
		}
		lpVertex->sx = (float)sx - w - gapsize;
		...
		lpVertex->sy = (float)sy - w - gapsize;
		lpVertex->sz = 0.99999f;
		lpVertex->rhw = 1.f;
		lpVertex->color = color;
		lpVertex->specular = 0xFF000000;
